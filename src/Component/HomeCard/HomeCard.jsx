...
const HomeCard = () => {
    ...
    return (
        <div data-theme={`${theme ? 'dark' : ''}`} className='px-4 mx-auto max-w-7xl sm:px-6 lg:px-8 py-10'>
            <h1 className="text-lg md:text-3xl font-bold leading-tight sm:text-4xl xl:text-5xl mb-10 text-center ">Assignmets</h1>
            {
                    ...
                    <>
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5'>
                            {assignmentData.slice(0, 3).map(data => (
                                <div key={data._id} className="relative border-2 border-blue-900 rounded-[1.5em] bg-gradient-to-br from-blue-900 via-blue-700/80 to-blue-500 text-white p-[1.5em] flex flex-col gap-[1em] justify-between">
                                    ...
                                    <Link to={`/details/${data._id}`}>
                                        <button className="w-full mt-2 px-4 py-2 bg-blue-900 rounded-lg">See more...</button>
                                    </Link>
                                </div>
                            ))}
                        </div>
                        ...
                        {assignmentData.length > 3 && (
                            <div className='mt-6 flex justify-center'>
                                <Link to="/assignment">
                                    <button className="px-6 py-2 bg-gradient-to-r from-blue-700 to-blue-500 text-white rounded-lg shadow-lg hover:opacity-90 transition">
                                        See All Assignments
                                    </button>
                                </Link>
                            </div>
